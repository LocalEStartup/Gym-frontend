...
function Sidebar({ isOpen, toggleSidebar, menuItems, setHeaderTitle }) {
  ...
  return (
    <div
      className={`bg-brand-cusprimary text-white w-64 fixed lg:static z-50 transform ${
        isOpen ? "translate-x-0" : "-translate-x-full"
      } lg:translate-x-0 transition-transform duration-300 h-full shadow-lg`}
    >
      <div className="p-4 font-bold text-lg flex justify-between items-center border-b border-brand-cussecondary">
              ...
              <li
                className={`flex items-center space-x-3 p-4 transition duration-200 ${
                  location.pathname.includes(item.path)
                    ? "bg-brand-cussecondary"
                    : "hover:bg-brand-cussecondary"
                }`}
              >
                <span>{item.icon}</span>
                <span>{item.name}</span>
